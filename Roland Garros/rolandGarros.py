...
def getStatus(txtArray):
    statusTag = '["tsp-lv","tsp-fm"]'
    i = 0
    for element in txtArray:
        i += 1
        if statusTag in element:
            return clearLine(txtArray[i])


def displayResult(curr_os, refreshRate=30, displayStats=False):
    """

    :param curr_os: current OS to be able to clear and so refresh console
    :param refreshRate: In seconds. Stats should not be updated with a very high frequency, so it is useless refresh it to frequently
    ...
    while(isInLive):
        ...
        if displayStats:
            ...
            for stat in getStats(contentLines):
                print(stat)
        if not('direct' in getStatus(contentLines).lower() or 'live' in getStatus(contentLines).lower()):
            isInLive=False
...
displayResult(currOs, refreshRate=15)
